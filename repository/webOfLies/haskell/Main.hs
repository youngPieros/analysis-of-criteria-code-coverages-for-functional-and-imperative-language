...
runScript = do
  ...
  testCases <- times numberOfTestCases testCase
  let results = map (\test -> solve test) testCases
  let testcaseOutputs = map (\r -> showInLine r) results
  let output = foldl (\acc res -> acc ++ res) "" testcaseOutputs
  return output
